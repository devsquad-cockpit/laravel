...
<x-cockpit::input.wrapper class="max-w-sm mx-auto {{ $wrapperClass }}"
    ...
    <div class="flex items-center space-x-3 text-gray-400 font-medium text-sm">
        <div class="relative flex items-center">
            <input type="text" name="{{ $name }}" id="{{ $id }}" x-ref="{{ $id }}" :value="minValue"
                   x-bind:class="{ 'bg-white' : !darkMode, 'bg-transparent' : darkMode}"
                    {{ $attributes->class([
                        'appearance-none block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none transition duration-150 ease-in-out sm:text-sm text-gray-700 dark:text-gray-400',
        ...
        <span class="text-dark-primary dark:text-gray-400">to</span>
        ...
        <div class="relative flex items-center">
            <input type="text" name="{{ $nameMax }}" id="{{ $idMax }}" x-ref="{{ $idMax }}" :value="maxValue"
                   x-bind:class="{ 'bg-white' : !darkMode, 'bg-transparent' : darkMode}"
                    {{ $attributes->class([
                        'appearance-none block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none transition duration-150 ease-in-out sm:text-sm text-gray-700 dark:text-gray-400',
                        'bg-transparent dark:bg-dark-secondary border-gray-300 dark:border-gray-500 placeholder-gray-400 focus:border-primary focus:ring-primary' => !$errors->has($nameMax),
